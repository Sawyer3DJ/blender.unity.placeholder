/*
 * Placeholder Switcher — v1.0.6
 * ------------------------------------------------------------
 * DIFF FROM v1.0.5
 * - Viewer: removed default cube; dark overlay when no matches (or prefix < 3).
 * - 3D Viewer controls row restored: control help + Invert Orbit X / Invert Orbit Y (Y on by default).
 * - Open Viewport / Close Viewport button (UI only stub) placed above Re-center View.
 * - “Auto-sync model view to viewport” large checkbox next to viewport control (UI only stub).
 * - Viewer Background row remains under the viewport (Current Sky / Unity Sky).
 * - Re-center View moved above Load/Save.
 * - Combine warning appears only when “Combine objects into one” is ticked.
 * - Scale Offset: “Adds to seed value” semantics fixed; Scale Clamping = min/max boxes + slider.
 * - Right-side vertical scrolling fixed (stateful).
 * - Overlay text clarified (≥3 chars & drag prefab tip).
 * - Version label updated to v1.0.6.
 *
 * NOTES
 * - Menu path: Tools > Placeholder Tools > Placeholder Switcher
 * - “Open/Close Viewport” & “Auto-sync …” are UI placeholders; wiring arrives in the next pass.
 */
    ...
    private void OnGUI()
    {
        ...
        using (new EditorGUILayout.HorizontalScope())
        {
            ...
            if (newOpen != viewportOpen)
            {
                ...
                // next pass will actually open/close a dedicated SceneView mirror window
            }
