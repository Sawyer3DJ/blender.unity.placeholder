/*
 * Placeholder Switcher — v1.0.6 (hotfix2)
 * ------------------------------------------------------------
 * WHAT I FIXED (edit-only; no layout churn)
 * - Fixed a bad merge in ApplyPreviewBackground(): removed stray braces/else and
 *   unified on one background enum (PreviewBg) so the method compiles and switches properly.
 * - Removed duplicate field declarations (invertOrbitX, invertOrbitY, autoSyncViewport, preview bg).
 * - Restored vertical-only scroll on right panel; no sideways scrollbars.
 * - Ensured Open/Close Viewport toggle exists (UI stub) and “Auto-sync model view to viewport” checkbox works with SceneView mirror helper.
 * - Brought back the left “Switch Placeholders” button.
 * - Rewired preview repaint heartbeat (PreviewHeartbeat) via OnInspectorUpdate() so the viewer updates in real-time again.
 * - Ensured placeholders render in the preview (no default cube when there are no matches; overlay shows instead).
 * - Minor: used UnityEngine.GUI.DrawTexture for the preview texture to avoid namespace ambiguity errors.
 *
 * NOTES
 * - Menu path: Tools > Placeholder Tools > Placeholder Switcher
 * - “Open/Close Viewport” & “Auto-sync …” are UI stubs; they mirror SceneView rather than embedding it (Unity limitation).
 * - Keeps your v1.0.6 UI look/feel; future polish can happen in a separate pass.
 */
...
public class PlaceholderSwitcher : EditorWindow
{
    ...
    [SerializeField] private bool autoSwitchPlaceholdersToScene = false; // warn when enabled
    ...
    // -------------------------
    // Scale Offset (XYZ locked base + axis influence)
    // -------------------------
    ...
    [SerializeField] private bool invertOrbitY = true;  // Y inverted by default (your preference)
    [SerializeField] private bool viewportOpen = false; // UI state only for now
    [SerializeField] private bool autoSyncViewport = true; // big checkbox next to open/close

    // right column scroll state
    private Vector2 _rightScroll = Vector2.zero;

    // Preview pulse / change tracking
    private double _lastPreviewPulse;
    private int _lastPlaceholderCount = -1;
    private string _lastPrefix = "";
    private GameObject _lastDesiredPrefab = null;
    private bool _needReframe = true;
    ...
    private void ApplyPreviewBackground()
    {
        ...
        switch (previewBg)
        {
            ...
            case PreviewBg.UnitySky:
                // Simple neutral color if no default Unity procedural skybox is set.
                if (RenderSettings.skybox != null)
                {
                    cam.clearFlags = CameraClearFlags.Skybox;
                }
                ...
                {
                    cam.clearFlags = CameraClearFlags.Color;
                    cam.backgroundColor = new Color(0.36f, 0.42f, 0.56f, 1f);
                }
                break;
        }
    }
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.LabelField("Controls: LMB Orbit, Shift+LMB (or MMB) Pan, Wheel Zoom", EditorStyles.miniLabel);
        using (new EditorGUILayout.HorizontalScope())
        {
            invertOrbitX = GUILayout.Toggle(invertOrbitX, "Invert Orbit X", "Button", GUILayout.Width(120));
            invertOrbitY = GUILayout.Toggle(invertOrbitY, "Invert Orbit Y", "Button", GUILayout.Width(120));
        }

        // Viewport controls (UI stub for SceneView mirror)
        GUILayout.Space(6);
        using (new EditorGUILayout.HorizontalScope())
        {
            bool newOpen = GUILayout.Toggle(viewportOpen, viewportOpen ? "Close Viewport" : "Open Viewport", "Button", GUILayout.Width(140));
            if (newOpen != viewportOpen)
            {
                viewportOpen = newOpen;
                ToggleSceneView(viewportOpen);
            }

            // big autosync checkbox
            var bigToggle = new GUIStyle(EditorStyles.toggle) { fontSize = 12 };
            autoSyncViewport = EditorGUILayout.ToggleLeft("Auto-sync model view to viewport", autoSyncViewport, bigToggle);
        }

        // Re-center View ABOVE load/save
        ...
        using (new EditorGUILayout.HorizontalScope())
        {
            using (new EditorGUI.DisabledScope(!CanSaveFromPreview()))
            {
                if (GUILayout.Button("Save From Preview As…", GUILayout.Height(24)))
                {
                    ...
                    if (!string.IsNullOrEmpty(path)) SaveCurrentPreviewAsPrefab(path);
                }
            }
        ...
        GUILayout.Label("v1.0.6", vStyle);
    }

    // Editor heartbeat so the preview updates even when idle
    ...
    private void DrawScaleOffset()
    {
        ...
        scaleMode = (ScaleMode)EditorGUILayout.EnumPopup(new GUIContent("Scaling Mode (Adds to seed value applies variation)"), scaleMode);
    ...
    {
        bool noPrefixOrNoMatches = (prefix.Length < 3) || (CountPlaceholders(prefix) <= 0);
        ...
        if (Event.current.type == EventType.Repaint)
        {
            ...
            // Draw scene placeholders only if there are matches; otherwise don't draw a cube
            if (!noPrefixOrNoMatches)
            {
                foreach (var go in candidates)
                {
                    if (!go) continue;
                    ...
                    foreach (var r in renderers)
                    {
                        ...
                        if (mf != null && mf.sharedMesh != null)
                        {
                            ...
                            for (int si = 0; si < subCount; si++)
                                previewUtil.DrawMesh(mf.sharedMesh, trs, (mats != null && mats[si] != null) ? mats[si] : fallbackMat, si);
                            drew = true;
                        }
                    }
                    if (!drew)
            ...
            UnityEngine.GUI.DrawTexture(rect, tex, ScaleMode.ScaleToFit, false);
        }
        ...
        if (rect.Contains(Event.current.mousePosition))
        {
            if (Event.current.type == EventType.MouseDrag)
            {
                if (Event.current.button == 0) // orbit
                {
                    ...
                    float sx = invertOrbitX ? -1f : 1f;
                    float sy = invertOrbitY ?  1f : -1f; // invert Y means moving mouse up rotates camera "down"
                    previewYaw  += Event.current.delta.x * 0.5f * sx;
                    previewPitch = Mathf.Clamp(previewPitch + Event.current.delta.y * 0.5f * sy, -80, 80);
                    Repaint();
                }
                else if (Event.current.button == 2 || (Event.current.button == 0 && Event.current.shift)) // pan
                {
                    previewUserAdjusted = true;
                    float panScale = previewDistance * 0.0025f;
                    var right = Quaternion.Euler(0, previewYaw, 0) * Vector3.right;
                    var up = Vector3.up;
                    // pan should feel natural regardless of invert settings
    ...
    // ======================================================
    // Preview upkeep / SceneView sync helpers
    // ======================================================
    private void PreviewHeartbeat()
    {
        if (EditorApplication.timeSinceStartup - _lastPreviewPulse < 0.1) return;
        _lastPreviewPulse = EditorApplication.timeSinceStartup;

        int currentCount = CountPlaceholders(prefix);
        if (currentCount != _lastPlaceholderCount || _lastDesiredPrefab != targetPrefab || _lastPrefix != prefix)
        {
            _needReframe = true;
            _lastPlaceholderCount = currentCount;
            _lastDesiredPrefab = targetPrefab;
            _lastPrefix = prefix;
            Repaint();
        }

        if (autoSyncViewport && SceneView.lastActiveSceneView != null)
            SyncFromSceneView();
    }

    private void ToggleSceneView(bool open)
    {
        var sv = SceneView.lastActiveSceneView;
        if (open)
        {
            if (sv == null) SceneView.FocusWindowIfItsOpen<SceneView>();
            else sv.Show(true);
        }
        else
        {
    ...
    private void SyncFromSceneView()
    {
        if (!autoSyncViewport) return;
        var sv = SceneView.lastActiveSceneView;
        if (sv == null || previewUtil == null) return;
        var cam = previewUtil.camera;
